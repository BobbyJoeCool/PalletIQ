 ...
 * HomePage.tsx's COLUMNS array — per the Main Menu Reorg (issue #174): Production, GPM
 * Functions, Inventory Management, Location Management, Container Management, Reporting
 * Functions (indices 0-5).
 */
test.describe('HomePage — menu restructure', () => {
  ...
  test('Reporting Functions shows SAR at the top and no "Other Reporting Functions" slot', async ({ page }) => {
    const reportColumn = column(page, 5);
  ...
  test('tapping SAR from the Reporting Functions column navigates to the built screen', async ({ page }) => {
    await column(page, 5).getByText('Staged Aisle Report').click();
  ...
  test('tapping ISI from the Location Management column navigates to the built screen', async ({ page }) => {
    await column(page, 3).getByText('Item Storage Inquiry').click();
