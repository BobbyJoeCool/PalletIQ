...
export interface PIPContainerScanResult {
  container: {
    id: string;
    pullFunction: string;
    quantity: PIPQty;
...
interface PIPContextValue {
  /** The most recently scanned pull container (plus its pallet/location), or null if none
   *  has been loaded this session yet. */
  containerData: PIPContainerScanResult | null;
  setContainerData: (c: PIPContainerScanResult | null) => void;
}

const PIPContext = createContext<PIPContextValue | null>(null);

/**
 * Session-level store for PIP's "currently scanned pull container" (App-Wide screen-
 ...
 * so navigating away from PIP and back restores the last-scanned container instead of
 * resetting to the empty Ready state. A stale persisted container (e.g. someone else pulled
...
export function PIPProvider({ children }: { children: React.ReactNode }) {
  const [containerData, setContainerData] = useState<PIPContainerScanResult | null>(null);
  return (
    <PIPContext.Provider value={{ containerData, setContainerData }}>
      {children}
    </PIPContext.Provider>
  );
}

/** Hook that returns the current PIP context: the last-scanned container. Must be called inside a PIPProvider; throws if used outside. */
