...
This app (the "floor app," used on the SYMBOL tablet) is one of several independently
versioned pieces in this monorepo — see the repo root [`README.md`](../../README.md)
for the full picture. From this app's own perspective:

- **`src/`** (this folder) — the React app, built with Vite.
- **`../../api/`** — Azure Functions, one per route, sharing a Prisma client singleton
  against a self-hosted MySQL instance (local demo phase — see GitHub #139). Its own
  independent version/changelog: [`../../api/CHANGELOG.md`](../../api/CHANGELOG.md).
- **`../../shared/`** — TypeScript types and constants imported by this app, the API,
  and the desktop app.
- **`../desktop-app/`** — the Manager-facing Desktop app (Electron-bundled web app),
  versioned and changelogged independently (see
  [`../desktop-app/CHANGELOG.md`](../desktop-app/CHANGELOG.md)) but sharing this same
  API and `shared/`. Scaffold only as of writing.

This app and the Azure Functions API run locally on the developer's laptop for the demo phase (see GitHub #139), the API served over the LAN to the tablet. Azure Static Web Apps hosting/deploy is not currently wired up (its GitHub Actions workflow was removed alongside the database migration); re-pointing at Azure SWA and/or Azure SQL remains straightforward if this goes to a hosted deployment later.
...
This app is one of three independently-versioned pieces in the monorepo (see the repo
root [`README.md`](../../README.md) for the full picture). From the repo root:

```
palletiq/
├── api/                       Azure Functions (Node/TypeScript) — own package.json/CHANGELOG.md
│   ├── functions/             One file per route
│   ├── lib/                   Prisma client, JWT, shared helpers
│   ├── prisma/                schema.prisma, migrations, seed scripts
│   └── host.json
├── apps/
│   ├── floor-app/             This app — the SYMBOL tablet build
│   │   ├── src/
│   │   │   ├── components/
│   │   │   ├── pages/
│   │   │   ├── context/       Auth, global state
│   │   │   └── lib/           Typed API client calls, formatting helpers
│   │   ├── tests/e2e/
│   │   ├── staticwebapp.config.json
│   │   └── vite.config.ts
│   └── desktop-app/           Manager desktop app (Electron) — scaffold only as of writing
└── shared/                    Types and constants used by api/ and both apps/
    ├── types/
    └── constants/
```
