...
6. **A Verify-Put Modal opens over the screen for this transaction** (issue #151) and the header's Back/Home/Jump/Logout remain disabled until it resolves — no other function can be navigated to while a put is in a Reserved state. The modal has two bodies: **Rack Put** (any Size but XS) shows just the pallet's info and Confirm Location; **Hand Put** (Size XS) additionally shows the pallet's Carton Quantity and, once one is found, an Exists Elsewhere option (see below).
7. The worker has up to four ways to resolve a Reserved state:
   - **Confirm:** scan the location barcode (system reads Aisle+Bin only, per Location Barcode Handling above). On match, the location's status changes from Reserved to Stored, the pallet's location record (and its inherited Storage Code/Size/Zone) updates, clearing any prior location automatically, and the modal closes. If the confirmed location was already `STAGED` when directed (step 4's preferred case), the message bar shows `Put complete — location {######}`; if the search fell through to an `EMPTY` location instead, the message bar shows the same text as informational (not success) with a note that the location wasn't staged.
   - **Unassign:** the worker presses a dedicated Unassign button if they want to back out without completing the put. The location returns to Empty (or Staged, if that's genuinely how it was found) and the modal closes.
   - **Hold Location:** if the directed location is physically inaccessible or otherwise unusable, the worker presses Hold Location, which opens the same hold-placement panel used elsewhere in the app (Warehouse Location Hold) to place any hold type they're permitted to place, with a reason code. Placing the hold releases the reservation as a side effect — the location is never re-searched or auto-redirected; the modal closes and the worker starts over from Aisle entry if they still need to put this pallet somewhere. (This replaced an earlier "Blocked Put" design that auto-redirected to a new location; that behavior was removed in favor of always stopping and letting the worker decide the next step themselves.)
   - **Exists Elsewhere** (Hand Put only, Inventory Managers and above): shown once the system finds the pallet's DPCI already stored at another Hand Put (XS) location. Tapping it opens a popup listing each match by location and current carton count; picking one releases the original reservation, then merges this pallet's cartons into the existing pallet at the picked location (a consolidation, not a second put) and closes the modal. Restricted to IM+ because consolidating onto an existing pallet is an IM+-only action everywhere else it appears in the app (see Manual Put's own consolidate resolution below).
8. **Automatic timeout:** if a Reserved location is not resolved within 5 minutes, the server automatically clears the reservation and returns the location to Empty. If the worker is still logged in, their screen updates to reflect this and the modal closes. If they are not (device reboot, crash, etc.), the location still clears on the server side regardless.
...
**Logged events** include, at minimum: put confirmed, pull confirmed, pallet moved, hold placed, hold removed, pallet field edited, location unassigned, pallet reinstated (one entry per pallet created, even when a single Pallet Reinstate submission creates several), and the Manual Put pallet-ID scan described above.
