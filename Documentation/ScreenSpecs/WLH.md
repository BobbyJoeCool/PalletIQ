...
7. Reason Code: **(v1.6.7, redesigned 2026-08-03 #84)** the shared `ReasonCodeField` — a department/role prefix letter plus a 2-digit reason number, each its own `CodePickerField`-based entry-with-dropdown-helper, fetched live per-user from `GET /api/reason-codes` rather than a hardcoded list. `strict` now rejects an unrecognized value instead of silently accepting it.
8. On **Confirm Hold**: if a *different* hold already exists on the location, a **Replace existing hold?** confirmation dialog appears first (**v1.6.10** — moved here from immediately after the Hold Type tap, since Reason Code is now filled in before Confirm rather than after); confirming (or, if there was no existing hold to begin with, tapping Confirm Hold directly) calls `PATCH /api/locations/:id/hold` with `{ holdType, reasonPrefix, reasonNumber }` (issue #84 — was one `reasonCode` string, now validated against the caller's own access and stored as real `ActivityLog` columns). On success, `HoldPanel` reloads the location's hold state, clears the selected type and reason code, the Message Bar shows a success line, `playAlert('info')` fires, and **(v1.6.10)** a one-line entry (`"Placed {type} on {location}"`) is appended to the Hold Log.
...
5. Worker picks **Bin Side** (All / Odd only / Even only, default All) and **Action** (Place / Release, default Place) via segmented controls. Switching Action resets whatever Hold Type/Release Level was selected (**v1.8.0, #123**) — a PLACE-only selection can't leak into Release's own option set or vice versa.
6. Worker also picks a **Hold Type** — a 2×2 grid of buttons (**v1.6.10**, matching Single Location's own grid shape) — for **both** actions now (**v1.8.0, #123**): for Place, only types the role can place are listed, labeled "Hold Type"; for Release, all 4 hold categories are listed, labeled **"Release Level"** — it's the level being released *from*, not a filter on what gets touched (see hierarchy below). A **Reason Code** (same entry-with-dropdown-helper `ReasonCodeField` as single-location, **v1.6.7**) is still Place-only — Release never needed one and still doesn't.
7. **Review Hold**/**Review Release** button enables once the range is valid (`startBin <= endBin`, all three numbers present, Level range either fully set or fully blank) and a Hold Type/Release Level is set — plus, for Place only, a reason code (**v1.8.0**: Release now also gates on Hold Type being set, previously it had no such field at all). Pressing it calls `GET /api/locations/range-count` (including `startLevel`/`endLevel` when set) to fetch the matching location count and opens a `ConfirmDialog` stating the exact range (including a "Levels {start}–{end}" clause when applicable), location count, and — for Release — which hold level is being released.
8. Confirming submits `PATCH /api/locations/range-hold` (Place) or `DELETE /api/locations/range-hold` (Release) with the same range/binSide/startLevel/endLevel plus `holdType` (both actions now, **v1.8.0**) and `reasonPrefix`/`reasonNumber` (Place only, issue #84).
9. On success: `playAlert('info')`, every field resets (Aisle/Start/End Bin and Start/End Level clear, Bin Side resets to All, Action resets to Place, Hold Type/Reason Code clear and the Reason Code field remounts to reset its own dropdown state). **(v1.6.10)** The Message Bar stays short — `"Placed {type} on {total} locations — {range description}"` (Place) or `"Holds released on {total} locations — {range description}"` (Release) — while the **full** per-bucket breakdown goes to the Hold Log instead, using the `breakdown` array both range endpoints now return:
   ...
   - Release log entry (**v1.8.0, #123**): one line per outcome bucket — `"{n} released (was {type})"`, `"{n} downgraded {existing} → {next}"` (a Both bucket losing only the requested direction, the opposite direction's hold stays in place), or `"{n} blocked (still {type}, insufficient role)"`. Buckets the requested level doesn't touch at all (`untouched`) aren't logged as an outcome line.

**Hold hierarchy — Range Place** (single-location Place always overwrites outright, since the worker sees exactly what they're replacing one location at a time; Range Place cannot, since it can silently touch many locations at once). Priority low→high: `HOLD_IN = HOLD_OUT < HOLD_BOTH < HOLD_PERM`.
...
**Hold hierarchy — Range Release (v1.8.0, #123)**. Release now requires a Release Level and is priority-aware — it no longer blindly clears every hold in the range regardless of type — via `resolveRangeRelease(requested, existing)` in `api/functions/locations.ts`, applied per-bucket against the snapshot taken before any writes (same snapshot-before-write pattern Place already used):

- Releasing `HOLD_PERM` clears **everything** at or below it in the range — Perm, Both, In, and Out buckets all outcome `released` — since nothing outranks Perm, releasing it removes any lock a lower hold type could have implied too. Each bucket's own role threshold (`HOLD_REMOVE_MIN_ROLE`) still applies independently — an IM releasing Perm gets In/Out/Both cleared but the Perm bucket itself `blocked` (needs Lead+).
- Releasing `HOLD_BOTH` clears Both **and** pure `HOLD_IN`/`HOLD_OUT` locations in the range (not just exact Both matches) — since Both is the union of In+Out, releasing it also releases either half held alone. `HOLD_PERM` locations are `untouched` (Perm outranks Both, symmetric to Place).
- Releasing `HOLD_IN` (or `HOLD_OUT`, symmetric): an exact `HOLD_IN` match is `released`; a `HOLD_BOTH` location is **downgraded** to `HOLD_OUT` (the opposite direction's hold survives, only the requested direction is removed) rather than being fully released; `HOLD_PERM` and the opposite pure direction are `untouched`.
- A downgrade requires the **same role threshold as fully removing the bucket being downgraded from** — e.g. downgrading a Both location via a Hold In release still checks `HOLD_REMOVE_MIN_ROLE.HOLD_BOTH`, not In's own (typically lower) threshold, since the location is still losing part of a Both-level hold.
- `untouched` buckets need no role check at all (nothing is being changed); `blocked` means the role check for that bucket's outcome failed.

**Retested live (v1.6.10)** against the local Docker DB, all outcomes confirmed correct: HI+HI stacking (placed), HI then HO on an overlapping range (upgrade to HB on the overlap, plain placed on the non-overlapping part), HP placement scoped to a single level via the new Level range filter (overwrites an HB underneath), a subsequent HI attempt against that same bin fully blocked (HP at one level, HB at the others — both outrank HI), Release across a mixed HI/HB/HO/HP range (exact per-bucket counts, pre-#123 flat-clear behavior), and an IM's Release correctly blocked on a HOLD_PERM bucket while an ADMIN's Release of the same bucket succeeds.

**Retested live (v1.8.0, #123)** against the local dev DB on a 5-location range (aisle 100, bins 1–5, level 1): all 4 Release Levels driven directly against `DELETE /api/locations/range-hold` and checked via SQL after each — In release downgraded Both→Out and left Perm untouched; Out release was the mirror image; Both release cleared pure In/Out plus Both, left Perm untouched; Perm release cleared everything including Perm itself. IM releasing Hold Permanent correctly released the In/Out/Both buckets while leaving Perm `blocked` (needs Lead+) — confirms per-bucket role gating, not a single check against the requested level.
...
│ │  CURRENT HOLD   None / …    Warehouse         │ │  25 placed (was   │ │
│ │  [Remove Hold]                Empty Location  │ │  None)   10:38 AM │ │
│ │   (Hold Reason, right of Current Hold,  │ │ ─────────────────│ │
│ │    same row, 2 lines, reserved height   │ │ (empty state:     │ │
│ │    so it never shifts — 2026-08-03)     │ │ "No hold actions  │ │
│ │  Hold Type (2×2 grid,        │ Reason Code:          │ │ "No hold actions  │ │
│ │   left, level with Reason    │  [W][▾] [01][▾]       │ │ yet this session")│ │
│ │   Code):                     │ [Confirm Hold][Cancel]│ │                   │ │
│ │   [Hold Inbound][Hold Outbound] (under Reason Code,   │ │                   │ │
│ │   [Hold Both]   [Hold Permanent] not spanning both cols)│ │                 │ │
│ │  Range mode:                                       │ │                   │ │
│ │  AISLE│START BIN END BIN│START LVL END LVL         │ │                   │ │
│ │  [___]│[___]      [___] │[__]      [__]  (÷ dividers)                    │ │
│ │  Bin Side: [All][Odd][Even]    Action: [Place][Release]                  │ │
│ │  Hold Type / Release Level (2×2 grid, both actions)                      │ │
│ │  Reason Code: [W][▾] [01][▾]  (Place only)                               │ │
...
- **(v1.6.7, redesigned 2026-08-03 #84)** Reason Code is the shared `ReasonCodeField` — now two `CodePickerField`-based parts (a department/role prefix letter, locked if the worker has only one accessible letter and session-sticky otherwise, plus a 2-digit reason number) and a read-only resolved-description display, both fetched live per-user from `GET /api/reason-codes` instead of a hardcoded list. `strict` is now set on both parts, closing the old free-text-bypass the flat design had.
...
**Writes:**
- `Location.holdCategory` — set on `PATCH /api/locations/:id/hold` (single Place/Replace) or cleared on `DELETE /api/locations/:id/hold` (single Remove); set/upgraded/cleared/downgraded in bulk via `PATCH`/`DELETE /api/locations/range-hold` (Range Place/Release), per the hierarchy rules above. Range Place/Release's `where` clauses now also intersect against `startLevel`/`endLevel` when supplied (**v1.6.10**).
- `ActivityLog` — one entry per action: `actionType: 'HOLD_PLACE'` (single place/replace, real `reasonPrefix`/`reasonNumber` columns per issue #84, plus `details: { holdType, previousHoldType }`), `actionType` for single remove (details: `{ clearedHoldType }`), `actionType: 'RANGE_HOLD'` (`reasonPrefix`/`reasonNumber` columns, `details: { startBin, endBin, binSide, startLevel, endLevel, holdType, placed, upgraded, blocked }`), `actionType: 'RANGE_REL'` (`details: { startBin, endBin, binSide, startLevel, endLevel, holdType, released, downgraded, blocked }`, **`holdType` and `downgraded` added v1.8.0, #123** — Release now takes a requested level as input the same way Place does, and downgrade is a distinct outcome from a full release) — **(v1.6.10)** both now also carry `startLevel`/`endLevel` when the Level range was used.
- **Not persisted anywhere:** `placeRangeHold`/`releaseRangeHold`'s response now also includes a `breakdown` array (one row per distinct existing-hold bucket, **v1.6.10**; each row's `outcome` can be `released`/`downgraded`/`blocked`/`untouched` as of **v1.8.0, #123** for Release) — this is response-only, purely to feed the client's session Hold Log; it is not written to `ActivityLog` or any other table (the aggregate counts already written there are unchanged).

**Not written to `Location`:** the reason code has never been stored as a column on `Location` itself — `Location` has no reason-code field at all. **(Issue #84, 2026-08-03)** It is now a real `ActivityLog.reasonPrefix`/`reasonNumber` column pair on the placing action, not just a `details` JSON blob — but there is still no per-location, cross-session "hold history" lookup outside the Activity Log overlay/table (and, for the current session only, the Hold Log panel — see Behind the Scenes).
...
flowchart TD
    ...
    R3 -- Release --> R5[Pick Release Level 2x2 grid<br/>#123]
    R4 --> R5b[Review Hold]
    R5 --> R5c[Review Release]
    R5b -->|GET range-count incl. level filter| R6[Confirm dialog: N locations]
    R5c -->|GET range-count| R6
    R6 -->|Confirm, PATCH/DELETE range-hold incl. holdType| R7[Short Message Bar summary,<br/>full released/downgraded/blocked breakdown<br/>appended to Hold Log, fields reset]
```
...
| Date | Change |
|---|---|
| 2026-08-03 ([#84](https://github.com/BobbyJoeCool/PalletIQ/issues/84) follow-up, direct instruction) | `HoldPanel`'s Hold Reason moved again: instead of its own full-width row above Current Hold, it now sits to the right of Current Hold in the same row, as two lines (department on one line, reason on the next — `ReasonCodeField`'s `onResolvedDescriptionChange` callback now reports the two parts separately instead of one joined string). That row reserves a fixed two-line height up front regardless of whether a code is picked yet, so nothing below it shifts down once the text actually populates. |
| 2026-08-03 ([#84](https://github.com/BobbyJoeCool/PalletIQ/issues/84) follow-up, direct instruction) | `HoldPanel` refined a third time the same day: Confirm Hold + Cancel moved from a full-width row below both columns to directly under the Reason Code entry box specifically (right column only). The resolved reason description ("Written Reason") moved out of `ReasonCodeField`'s own layout entirely — new `descriptionPosition="none"` value plus a new `onResolvedDescriptionChange` callback let `HoldPanel` lift the text into its own state and render it once, in larger text (20px vs. the usual 13px), at the very top of the popup, above Current Hold. |
| 2026-08-03 ([#84](https://github.com/BobbyJoeCool/PalletIQ/issues/84) follow-up, direct instruction) | `HoldPanel` refined a second time the same day: Current Hold/Remove Hold moved back to their own full-width row above the two-column split, so Hold Type (left) and Reason Code (right) both start fresh and land level with each other regardless of whether Remove Hold rendered; Confirm Hold and the renamed **Cancel** button (was Close) moved to sit side by side below both columns instead of Close stacking under Confirm Hold in the right column alone; `ReasonCodeField` gained a `descriptionPosition` prop (`'above'`, used here) so its resolved description reads as a heading over the code pickers rather than a footnote under them. SDP's Hold Location no longer uses `HoldPanel` at all as of the entry below — this change affects WLH, PIP, MNP only now. |
| 2026-08-03 ([#84](https://github.com/BobbyJoeCool/PalletIQ/issues/84) follow-up) | `HoldPanel`'s Single Location layout changed from one vertical stack to two side-by-side columns (direct instruction, "more horizontal"): Current Hold/Remove Hold/Hold Type grid stay on the left; Reason Code and Confirm Hold (plus Close, where shown) move to a right column. Card anchors top-left (`ModalOverlay`'s new `position="top-left"`) on every quick-hold modal that wraps `HoldPanel` (PIP, MNP, SDP's Verify-Put Modal) so neither the on-screen Keyboard nor Numpad covers it; outer width grew `max-w-[520px]` → `max-w-[760px]` to fit both columns. Shared component — affected WLH, PIP, MNP, SDP identically at the time (SDP later moved off `HoldPanel` — see the entry above). |
| 2026-08-03 ([#84](https://github.com/BobbyJoeCool/PalletIQ/issues/84)) | Reason Code (single-location and Range Place) redesigned onto the new database-backed system — a department/role prefix letter (session-sticky, locked if the worker has only one accessible letter) plus a 2-digit reason number, both fetched live per-user from `GET /api/reason-codes`, replacing the flat hardcoded `HOLD_REASON_CODES` list. `strict` now closes the old free-text bypass. Submits `reasonPrefix`/`reasonNumber` instead of one `reasonCode` string; both are now real `ActivityLog` columns, validated server-side against the caller's own access on every submit. Made once in the shared `ReasonCodeField`/`HoldPanel` components — WLH, PIP, MNP's quick-hold panels, SDP's Verify-Put Modal Hold Location, and STG's reject dialog all picked up the change automatically. |
